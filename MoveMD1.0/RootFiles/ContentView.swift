...
struct ContentView: View {
        ...
        buildContent()
            .onAppear { 
                ...
                if authManager.currentAppleUser == nil {
                     ...
                     logger.info("[ContentView] Root content onAppear: currentAppleUser exists (Name: \(authManager.currentAppleUser?.name ?? "N/A")). isUnlocked: \(purchaseManager.isUnlocked)") 
                }
            }
    }
    ...
    private func buildContent() -> some View {
        if authManager.currentAppleUser == nil {
            ...
            let _ = logger.debug("[ContentView] CurrentAppleUser exists. Checking purchase/trial status (isUnlocked: \(purchaseManager.isUnlocked)).")
            if purchaseManager.isUnlocked {
                let _ = logger.debug("[ContentView] User is unlocked.")
                if let user = authManager.currentAppleUser {
                    let _ = logger.debug("[ContentView] User ID: \(user.appleUserId?.prefix(8) ?? "N/A"), OnboardingComplete: \(user.isOnboardingComplete)")
                    if user.isOnboardingComplete {
                        let _ = logger.debug("[ContentView] User onboarding complete. Showing WorkoutListScreen.")
                        WorkoutListScreen() 
                            .onAppear {
                ...
                let _ = logger.debug("[ContentView] User is NOT unlocked. Showing PurchaseView.")
...
struct ContentView_Previews: PreviewProvider {
    @MainActor static var previews: some View {
        ...
        let purchaseManager_unlocked = PurchaseManager.shared 
        let purchaseManager_locked = PurchaseManager.shared 
        ...
        return Group {
            ContentView()
                ...
                .environmentObject(purchaseManager_unlocked)
            ...
            ContentView()
                .environmentObject(authManager_loggedIn_onboarded) 
                .environmentObject(purchaseManager_locked)
                .modelContainer(container)
